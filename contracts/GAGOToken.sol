...
contract GAGOToken is BEP1155, Ownable {

    mapping(uint256 => string) private _uris;
    constructor() {}

    function mintRealEstate(address to, uint256 id, uint256 amount, string memory uri_) public onlyOwner returns(bool) {
        _setURIToken(id, uri_);
        _mint(to, id, amount, "0x00");
        return true;
    }   

    function mintRealEstateBatch(address to, uint256[] memory ids, uint256[] memory amounts, string[] memory uris, bytes memory data) public onlyOwner returns(bool) {
    ...
    function uri(uint256 tokenId) public virtual override view returns(string memory) {
        return(_uris[tokenId]);
    }

    function _setURIToken(uint256 tokenId, string memory uri_) internal {
